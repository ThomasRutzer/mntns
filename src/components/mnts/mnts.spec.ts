...
import store, {actions, actionTypes, mutationTypes, mutations} from './../../store';
...
describe('Mnts component', () => {
    ...
    describe('method focusedMnt()', () => {
        it(`selects matching dataSet`, async () => {
            ...
            await directiveTest.execute((vm) => { // ensure Vue has bootstrapped/run change detection
                ...
                childComp.focusedMnt({objectId: "1"});

                expect(store.state.gitHubData.focusedRepo.raw).to.equal(rawRepos[0])
            });
        });

        it(`resets store data when Ids don´t match`, async () => {
            let childComp: MockMntsComponent = null;
            directiveTest.createComponent({store});
            ...
            await directiveTest.execute((vm) => { // ensure Vue has bootstrapped/run change detection
                debugger;
                childComp = vm.$children[0];
                childComp.focusedMnt({objectId: "100"});
